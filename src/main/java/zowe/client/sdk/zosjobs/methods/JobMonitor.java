...
import zowe.client.sdk.zosjobs.input.JobGet.JobInputData;
import zowe.client.sdk.zosjobs.input.JobMonitor.JobWaitInputData;
...
public class JobMonitor {
     ...
     * @author Frank Giordano
     */
    private boolean checkMessage(final JobWaitInputData params, final String message) throws ZosmfRequestException {
        final JobGet getJobs = new JobGet(connection);
        final JobInputData filter =
                new JobInputData.Builder("*")
     ...
     * @author Frank Giordano
     */
    private CheckJobStatus checkStatus(final JobWaitInputData params) throws ZosmfRequestException {
     ...
     * @author Frank Giordano
     */
    private CheckJobStatus checkStatus(final JobWaitInputData params, final boolean isStepData)
     ...
     * @author Frank Giordano
     */
    public boolean isRunning(final JobWaitInputData params) throws ZosmfRequestException {
     ...
     * @author Frank Giordano
     */
    private boolean pollByMessage(final JobWaitInputData params, final String message) throws ZosmfRequestException {
     ...
     * @author Frank Giordano
     */
    private Job pollByStatus(final JobWaitInputData params) throws ZosmfRequestException {
        final int timeoutVal = params.getWatchDelay().orElse(DEFAULT_WATCH_DELAY);
    ...
    public boolean waitByMessage(final Job job, final String message) throws ZosmfRequestException {
        ...
        return waitMessageCommon(new JobWaitInputData.Builder(
    ...
    public boolean waitByMessage(final String jobName, final String jobId, final String message) throws ZosmfRequestException {
        ...
        return waitMessageCommon(new JobWaitInputData.Builder(jobName, jobId)
                .jobStatus(JobStatus.Type.OUTPUT)
                .attempts(attempts)
                .watchDelay(watchDelay)
                .build(), message);
    ...
    public Job waitByOutputStatus(final Job job) throws ZosmfRequestException {
        ValidateUtils.checkNullParameter(job == null, "job is null");
        return waitStatusCommon(new JobWaitInputData.Builder(
                job.getJobName().orElseThrow(() -> new IllegalArgumentException(JobsConstants.JOB_NAME_ILLEGAL_MSG)),
                job.getJobId().orElseThrow(() -> new IllegalArgumentException(JobsConstants.JOB_ID_ILLEGAL_MSG)))
                .jobStatus(JobStatus.Type.OUTPUT)
                .attempts(attempts)
    ...
    public Job waitByOutputStatus(final String jobName, final String jobId) throws ZosmfRequestException {
        ValidateUtils.checkIllegalParameter(jobName, "jobName");
        ValidateUtils.checkIllegalParameter(jobId, "jobId");
        return waitStatusCommon(new JobWaitInputData.Builder(jobName, jobId)
                .jobStatus(JobStatus.Type.OUTPUT)
                .attempts(attempts)
                .watchDelay(watchDelay)
                .build());
    ...
    public Job waitByStatus(final Job job, final JobStatus.Type statusType) throws ZosmfRequestException {
        ...
        return waitStatusCommon(
                new JobWaitInputData.Builder(job.getJobName().orElse(""), job.getJobId().orElse(""))
    ...
    public Job waitByStatus(final String jobName, final String jobId, final JobStatus.Type statusType)
            throws ZosmfRequestException {
        return waitStatusCommon(
                new JobWaitInputData.Builder(jobName, jobId)
                        .jobStatus(statusType)
                        .attempts(attempts)
                        .watchDelay(watchDelay)
                        .build());
     ...
     * @author Frank Giordano
     */
    public boolean waitMessageCommon(final JobWaitInputData params, final String message)
            throws ZosmfRequestException {
        ValidateUtils.checkNullParameter(params == null, "params is null");
        ValidateUtils.checkIllegalParameter(message, "message");
     ...
     * @author Frank Giordano
     */
    public Job waitStatusCommon(final JobWaitInputData params) throws ZosmfRequestException {
        ValidateUtils.checkNullParameter(params == null, "params is null");
